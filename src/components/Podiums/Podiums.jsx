...
import { podiumsData } from "./data";

function Podiums() {
  const { data, loading, error } = podiumsData;

  if (loading && !data) return <Loading />;
  if (error) return <Error error={error} />;
  const { competition } = data;

  const [finishedPodiums, nonfinishedPodiums] = partition(competition.podiums, (podium) => podium.round.finished);
      ...
      {finishedPodiums.length > 0 ? (
        <Grid container direction="column" spacing={2}>
          ...
          {nonfinishedPodiums.length > 0 && (
            <Grid item>
              <Typography gutterBottom>{`Podiums for the following events are yet to be determined:`}</Typography>
